...
setContact((prv)=>{
  ...
  return (
    <div className='flex justify-center md:items-center my-10 md:my-0 h-screen max-w-6xl mx-3 xl:mx-auto'>
        <div className='text-left w-full mt-8  mx-auto'>
            <h1 className='md:text-3xl my-3 text-2xl tracking-widest text-left'>Lets <span className='text-accent'>.Connect</span></h1>
            <p className='text-gray-400 tracking-wider text-sm my-3 mb-10 text-center xl:text-left'>"Feel free to email me at <Link href={"mailto:vinaykumar4008@gmail.com"} className='underline decoration-accent'>vinaykumar4008@gmail.com </Link>or use the contact form."</p>
            ...
            <textarea placeholder='Message' type='text' value={contact.message} onChange={(e)=>{handleChange(e)}} required name='message' className='p-2 w-full bg-transparent outline-none focus:border-b-[2px]  focus:border-b-accent shadow-lg border-b-2  text-white duration-300 transition-all border-b-[#332267]' rows={6}  />
            <button onClick={sendMail}
            disabled={spinner}
            className={`${spinner && "cursor-not-allowed"} relative btn flex justify-center items-center group border-2 border-[#332267] duration-300 transition-all hover:border-accent overflow-hidden w-max rounded-full py-3 px-8`}>
